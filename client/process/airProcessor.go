...
func (up *AirProcessor) WatchAir() (err error) {
	...
	fmt.Println(resMsg)
	fmt.Println(resMsg.AirConditioners)

	return
}


func (up *AirProcessor) GetReport() (err error) {
	...
	// 1，创建一个msg
	var msg message.Message
	msg.Type = message.TypeGetReport
	...
	// 反序列化Data
	var resMsg message.GetReportRes
	err = json.Unmarshal([]byte(msg.Data), &resMsg)
	if err != nil {
		fmt.Println("json.Unmarshal err=", err)
		return
...
func (up *AirProcessor) GetDetail(getDetail message.GetDetailList) (err error) {
	...
	// 1，创建一个msg
	var msg message.Message
	msg.Type = message.TypeGetDetailList

	// 4，将smsMsg序列化
	data, err := json.Marshal(getDetail)
	if err != nil {
		fmt.Println("getDetail json.Marshal err=", err)
		return
	}
	...
	// 反序列化Data
	var resMsg message.GetDetailListRes
	err = json.Unmarshal([]byte(msg.Data), &resMsg)
	if err != nil {
		fmt.Println("json.Unmarshal err=", err)
		return
...
func (up *AirProcessor) StopWind(stopWind message.AirConditionerStopWind) (err error) {
	...
	// 1，创建一个msg
	var msg message.Message
	msg.Type = message.TypeAirConditionerStopWind

	// 4，将smsMsg序列化
	data, err := json.Marshal(stopWind)
	if err != nil {
		fmt.Println("stopWind json.Marshal err=", err)
		return
	}
